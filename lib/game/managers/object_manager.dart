...
class ObjectManager extends Component with HasGameRef<DoodleDash> {
  ...
  void onMount() {
    ...
    // use the _semiRandomPlatform method to generate starting platforms and add it to the game.
    for (var i = 0; i < 9; i++) {
      if (i != 0) {
  ...
  void update(double dt) {
    ...
    if (topOfLowestPlatform > screenBottom) {
      ...
      // use the _semiRandomPlatform method to generate a platform and add it to the game.
  ...
  void enableLevelSpecialty(int level) {
    // More on Platforms: Add switch statement to enable:
    // SpringBoard for level 1
    // BrokenPlatform for level 2
    // NooglerPlatform for level 3
    switch (level) {
      case 1:
  ...
  // the configure method enables the Doodle Dash game to reconfigure the minimum and maximum distance between platforms and enables specialty platforms when the difficulty level increases
  void configure(int nextLevel, Difficulty config) {
    // change distance between platforms
  ...
  void _maybeAddPowerup() {
    ...
    } else if (specialPlatforms['ufo'] == true &&
        probGen.generateWithProbability(15)) {
      var ufo = UfoShip(
